...
/*
    @Test
    void should_update_existing_client() throws Exception {

        Client existingClient = new Client();
        existingClient.setName("ExistingClient");
        existingClient.setDescription("ExistingDescription");
        existingClient.setEmail("existing@gmail.com");
        existingClient.setPhoneNumber("1234567890");
        existingClient.setDomainName("existingdomain");
        existingClient.setActive(true);
        existingClient.setType("existingType");
        existingClient = clientRepository.save(existingClient);

        existingClient.setActive(false);

        ClientDTO dto = getMapper().toDTO(existingClient);
        dto.setActive(false);

        this.mockMvc
                .perform(put(CLIENT_URL + "/{id}", existingClient.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(toJson(dto).orElse("")))
                .andDo(print())
                .andExpect(status().isOk());
...
//        assertEquals("updated@gmail.com", updatedEntity.getEmail());
//        assertEquals("9876543210", updatedEntity.getPhoneNumber());
//        assertEquals("updateddomain", updatedEntity.getDomainName());
        assertEquals(false, updatedEntity.getActive());
