...
public class ClientProfileSpecification<T extends BaseClientEntity> implements Specification<T> {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        case GREATER_THAN:
            if (searchCriteria.getFilterKey().equals("deptName")) {
                System.out.println(searchCriteria.getValue());
                cb.greaterThan(departmentJoin(root).<Number> get(searchCriteria.getFilterKey()).as(long.class),
                        ((Number) searchCriteria.getValue()).longValue());
            }
            if (searchCriteria.getValue() instanceof Comparable<?>) {
                System.out.println(searchCriteria.getValue());
                return cb.greaterThan(root.<Comparable> get(searchCriteria.getFilterKey()),
                        (Comparable) searchCriteria.getValue());
            }
